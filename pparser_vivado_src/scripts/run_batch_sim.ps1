...
$snapshotName = "pparser_hw_static_tb_snapshot_stage2"
...
try {
    ...
    Invoke-VivadoStep -Tool "$vivadoBin\xvlog.bat" -CommandArgs @(
        ...
        "$projectDir\tb\pparser_hw_static_tb.v"
    )

    Invoke-VivadoStep -Tool "$vivadoBin\xelab.bat" -CommandArgs @(
        "work.pparser_hw_static_tb", "-L", "cam", "-L", "xpm", "-debug", "typical", "-s", $snapshotName
    )
