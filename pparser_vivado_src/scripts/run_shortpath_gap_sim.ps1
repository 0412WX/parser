...
$snapshotName = "pparser_shortpath_gap_tb_snapshot"
...
try {
    ...
    Invoke-VivadoStep -Tool "$vivadoBin\xvlog.bat" -CommandArgs @(
        ...
        "$projectDir\tb\pparser_shortpath_gap_tb.v"
    )

    Invoke-VivadoStep -Tool "$vivadoBin\xelab.bat" -CommandArgs @(
        "work.pparser_shortpath_gap_tb", "-L", "cam", "-L", "xpm", "-debug", "typical", "-s", $snapshotName
    )
