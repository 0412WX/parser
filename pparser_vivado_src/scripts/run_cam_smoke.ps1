...
try {
    ...
    Invoke-VivadoStep -Tool "$vivadoBin\xvlog.bat" -CommandArgs @(
        "-sv",
        "$projectDir\tb\pparser_cam_smoke_tb.v"
    )

    Invoke-VivadoStep -Tool "$vivadoBin\xelab.bat" -CommandArgs @(
        "work.pparser_cam_smoke_tb", "-L", "cam", "-debug", "typical", "-s", "pparser_cam_smoke_tb_snapshot"
    )
    Invoke-VivadoStep -Tool "$vivadoBin\xsim.bat" -CommandArgs @("pparser_cam_smoke_tb_snapshot", "-runall")
}
finally {
    Pop-Location
}
