...
try {
    ...
    Invoke-VivadoStep -Tool "$vivadoBin\xvlog.bat" -CommandArgs @(
        ...
        "$projectDir\tb\pparser_debug_tb.v"
    )

    Invoke-VivadoStep -Tool "$vivadoBin\xelab.bat" -CommandArgs @(
        "work.pparser_debug_tb", "-L", "cam", "-L", "xpm", "-debug", "typical", "-s", "pparser_debug_tb_snapshot"
    )
    Invoke-VivadoStep -Tool "$vivadoBin\xsim.bat" -CommandArgs @("pparser_debug_tb_snapshot", "-runall")
}
finally {
    Pop-Location
}
